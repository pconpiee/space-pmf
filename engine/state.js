Session 3 of 6 — `engine/state.js`
...
Commit message: `feat: engine/state.js — act derivation, segment evidence accumulation, per-act burn, pivot cost, failure states, win conditions`

---

**Three things to note before Session 4:**

**One — `applyDailyBurn` is day-accurate across act boundaries.** If an action costs 8 days and spans the Day 30→31 boundary, it charges Act I burn for Day 30 and Act II burn for Days 31–38. This means crossing an act boundary mid-action is handled correctly without approximation.

**Two — D30 retention has two paths.** If the student ran `retention_analysis` or `sean_ellis_survey`, the outcome object from that action carries a `retention` field that overrides the estimate. If they never ran a retention action, `computeD30Retention()` returns `null` — and the premature scaling failure check requires `d30Retention !== null` before firing. This means you can't trigger premature scaling failure by accident without having gathered any retention data.

**Three — segment evidence never resets on pivot.** A pivot changes `activeSegmentId` and costs time/budget. But `segmentEvidence` is preserved — the learning from Act I doesn't disappear just because you changed direction. The debrief can show "here's what you knew about each segment" regardless of which one you chose to build for.

Ready for Session 4 — `engine/logic.js`.
